...
export default function OrientacoesTemplatesContent() {
  ...
  const loadTemplates = async () => {
    const allData = await base44.entities.OrientacoesTemplate.list("-created_date");
    const user = await User.me();
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.is_default) {
      const allTemplates = await base44.entities.OrientacoesTemplate.list();
      for (const t of allTemplates) {
        if (t.is_default && t.id !== editingTemplate?.id) {
          await base44.entities.OrientacoesTemplate.update(t.id, { is_default: false });
        }
      }
    }
    
    if (editingTemplate) {
      await base44.entities.OrientacoesTemplate.update(editingTemplate.id, formData);
    } else {
      await base44.entities.OrientacoesTemplate.create(formData);
    }
  ...
  const handleDelete = async () => {
    if (deleteTemplate) {
      await base44.entities.OrientacoesTemplate.delete(deleteTemplate.id);
    ...
    <div className="p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Modelos de Orientações</h1>
            <p className="text-gray-600 mt-1">Gerencie seus modelos de orientações ao paciente</p>
          </div>
        </div>
        ...
        {templates.length === 0 && (
          <Card className="shadow-md border-none">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-gray-500 mb-6">Crie modelos para agilizar suas orientações</p>
              <Button
                onClick={() => setShowForm(true)}
        ...
        <Dialog open={showForm} onOpenChange={handleCloseForm}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <Label htmlFor="logo">Logotipo (opcional - 3x3cm)</Label>
